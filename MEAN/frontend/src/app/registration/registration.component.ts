...
*/
  ...
  async register() {
    ...
    let passwordRegex =
      /^((([a-z])(?=.*\d)(?=.*(\~|\`|\!|\@|\#|\$|\%|\^|\&|\*|\(|\)|\-|\_|\+|\=|\{|\}|\[|\]|\||\\|\/|\:|\;|\'|\"|\<|\>|\,|\.|\?))(?=.*[A-Z]))|(([A-Z])(?=.*\d)(?=.*(\~|\`|\!|\@|\#|\$|\%|\^|\&|\*|\(|\)|\-|\_|\+|\=|\{|\}|\[|\]|\||\\|\/|\:|\;|\'|\"|\<|\>|\,|\.|\?))).{6,})$/;

    let mailRegex = /^\w+@\w+\.\w+$/;
    //let gradDrzavaRegex=/^[A-Z]( |[[:alpha:]])*$/;

    if (
      this.emptyField(this.user.name) ||
      this.emptyField(this.user.username) ||
      this.emptyField(this.user.password) ||
      this.emptyField(this.user.birthday) ||
      this.emptyField(this.user.city) ||
      this.emptyField(this.user.country) ||
      this.emptyField(this.user.email) ||
      this.emptyField(this.passwordConfirm)
    ) {
    ...
    if (!passwordRegex.test(this.user.password)) {
      this.poruka =
        "Lozinka nije u dobrom formatu! Lozinka mora poceti slovom, sadrzati minimum 7 karaktera, medju kojima mora biti 1 broj, 1 veliko slovo i 1 specijalni karakter!";
      this.a = false;
      return;
    }
    ...
    if (this.user.password != this.passwordConfirm) {
      this.poruka =
        "Lozinka se ne poklapa sa ponovljenom lozinkom! Molim Vas popunite opet!";
      this.a = false;
      return;
    }
    ...
    for (; i < this.requests.length; i++) {
      if (
        this.user.username == this.requests[i].username ||
        this.user.email == this.requests[i].email
      ) {
        this.poruka = "Korisnicko ime ili email adresa su vec zauzeti! ";
        this.a = false;
        return;
    ...
    let promise = new Promise((resolve, reject) => {
      this.userService
        .register(this.user, this.fileToUpload)
        .subscribe((user: User) => {
          if (user) {
