...
export class HappeningComponent implements OnInit {
  ...
  async closeHappening() {
    if (this.element.active) {
      await new Promise((resolve, reject) => {
        this.userService
          .updateHappening(this.element.id, false)
  ...
  async openHappening() {
    ...
    if (this.element.dateEnd != null) {
      ...
      if (dateEnd.getTime() > Date.now()) {
        await new Promise((resolve, reject) => {
          this.userService
            .updateHappening(this.element.id, true)
            .subscribe((data: boolean) => {
              if (data) {
                resolve();
  ...
  async followRequest() {
    ...
    creator.followHappeningRequests.push(
      this.user.username + " " + this.element.id
    );
  ...
  async followHappening() {
    ...
    if (
      this.element.type == "privatno" &&
      this.element.creator != this.user.username &&
      this.element.active
    ) {
    ...
    this.userService
      .followHappening(this.element)
      .subscribe((data: boolean) => {
        if (data) alert("Uspesno pratite dogadjaj!");
        else alert("Doslo je do greske!");
      });
  }
    ...
    this.userService.updateHapReq(this.user).subscribe((data1: boolean) => {
      if (data1) {
        this.userService.findUser(username).subscribe((data2: User) => {
          ...
          if (this.element.active == false) {
            data2.notifications.push(
              "Dogadjaj " +
                this.element.name +
                " za koji ste poslali zahtev je zatvoren i ne mozete ga pratiti"
            );

            r = false;
          } else {
            data2.notifications.push(
              "Vas zahtev za pracenje dogadjaja " +
                this.element.name +
                " je prihvacen!"
            );
          ...
          this.userService
            .updateNotifications(data2)
            .subscribe((data3: boolean) => {
              if (data3) {
                if (r) {
                  this.userService
                    .followHappening(this.element)
    ...
    this.bookService.addNewComment(this.newComment).subscribe(async (data) => {
      if (data) {
        ...
        for (let i = 0; i < this.element.followers.length; i++) {
          ...
          await new Promise((resolve, reject) => {
            this.userService
              .findUser(this.element.followers[i])
              .subscribe((data: User) => {
                follower = data;
                resolve();
              });
          ...
          follower.notifications.push(
            "Korisnik " +
              follower.username +
              "je postavio komentra o dogadjaju " +
              this.element.name +
              " koga pratite "
          );
