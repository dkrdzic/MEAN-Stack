...
if (this.emptyField(this.book.name)||this.emptyField(this.authors) || this.emptyField(this.bookGanres)||
  ...
  for (let i=0;i<this.books.length;i++){
      ...
      for(let j=0;j<this.books[i].ganres.length;j++)
      {
        ...
        if(this.ganreToDelete==this.books[i].ganres[j]){
          ...
          this.poruka="Zanr "+this.ganreToDelete+" se ne moze obrisati jer postoji knjiga sa tim zanrom!";
          this.a=false;
          return;
        }
...
this.bookService.deleteGanre(this.ganreToDelete).subscribe(async (data:boolean)=>{
...
this.bookService.changeBookName(this.bookToChange,this.newBookName).subscribe(async (data:boolean)=>{
  if(data){
...
changeBookAuthors(){
  ...
  for (let i:number=0;i<autori.length;i++)this.bookToChange.authors.push(autori[i]);
  
  
  this.bookService.changeBookAuthors(this.bookToChange).subscribe(async (data:boolean)=>{
    if(data){
...
this.bookService.changeCover(this.bookToChange,this.fileToUpload).subscribe(async(data:boolean)=>{
...
if (this.bookGanres==null){
  ...
  return;
    ...
    for (let i:number=0;i<this.bookGanres.length;i++)this.bookToChange.ganres.push(this.bookGanres[i]);


    this.bookService.changeBookGanres(this.bookToChange).subscribe(async (data:boolean)=>{
      if(data){
  ...
  changeBookDescription(){
    ...
    this.bookService.changeBookDescr(this.bookToChange,this.newDescr).subscribe(async (data:boolean)=>{
      if(data){
    ...
    changeBookDate(){
      ...
      this.bookService.changeBookDate(this.bookToChange,this.newDate).subscribe(async (data:boolean)=>{
        if(data){
      ...
      changeBookPages(){
        ...
        this.bookService.changeBookPages(this.bookToChange,this.newPages).subscribe(async (data:boolean)=>{
          if(data){
