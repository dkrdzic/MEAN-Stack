...
return;
  ...
  this.bookService.updateBookPage(this.user.username,this.book.id,this.pages).subscribe((data:boolean)=>{
...
let i:number;
 for ( i=0;i<this.user.booksReading.length;i++){
      ...
      if (this.book.id==this.user.booksReading[i]){break;}
...
this.poruka="Da biste uneli broj stranica morate prvo zapoceti citanje knjige";
  this.a=false;
  return;
 }
...
this.pagesBool=true;
 ...
 this.bookService.findPages(this.user.username,this.book.id).subscribe(async (page:Page)=>{
...
async showMakeComment(){
 ...
 this.poruka="Da biste ostavili komentar morate prvo zapoceti citanje knjige i procitati bar 50% knjige";
   this.a=false;
   return;
  }
...
await new Promise((resolve,reject)=>{
  ...
  console.log("Page nije nadjen")
      ...
      this.currentPage=newPage;
      await new Promise((resolve,reject)=>{
        this.bookService.makeNewPage(newPage).subscribe((data:boolean)=>{
...
this.a=false;
this.poruka="Ne mozete ostaviti komentar ako niste procitali bar pola knjige";
return;
...
this.users[i].notifications.push("Korisnik "+this.user.username+" koga pratite je postavio komentar o knjizi "+this.book.name);

await new Promise((resolve,reject)=>{

this.userService.updateNotifications(this.users[i]).subscribe((data:Boolean)=>{
...
if (data){
  ...
  this.users[i].notifications.push("Korisnik "+this.user.username+" koga pratite je obrisao komentar o knjizi "+this.book.name);
...
this.bookService.changeComment(this.commentToChange,this.updateComment).subscribe(async (data:boolean)=>{
...
if(data){
  ...
  this.users[i].notifications.push("Korisnik "+this.user.username+" koga pratite je promenio komentar o knjizi "+this.book.name);
