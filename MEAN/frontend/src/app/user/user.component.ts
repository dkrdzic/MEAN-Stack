...
export class UserComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    for (let i = 0; i < this.allComments.length; i++) {
      ...
      let p1 = new Promise((resolve, reject) => {
        this.bookService
          .findBook(this.allComments[i].idBook)
          .subscribe((book: Book) => {
            if (book) {
              this.commentedBooks.push(book);
  ...
  searchUser() {
    ...
    if (
      this.emptyField(this.userSearchEmail) &&
      this.emptyField(this.userSearchName) &&
      this.emptyField(this.userSearchSurname) &&
      this.emptyField(this.userSearchUsername)
    ) {
    ...
    } else if (this.userSearchName) {
      for (let i = 0; i < this.users.length; i++) {
        if (this.users[i].name == this.userSearchName) {
          this.foundUsers.push(this.users[i]);
        }
      }
    } else if (this.userSearchSurname) {
      for (let i = 0; i < this.users.length; i++) {
  ...
  changeImg() {
    ...
    this.userService
      .changeImg(this.user, this.fileToUpload)
  ...
  showReadingBooks() {
    ...
    for (let i = 0; i < this.user.booksReading.length; i++) {
      for (let j = 0; j < this.books.length; j++) {
        if (this.user.booksReading[i] == this.books[j].id) {
          this.readingBooks.push(this.books[j]);
          break;
        }
      }
    }
    ...
    if (this.readingBooks.length == 0) {
      alert("Lista knjiga za citanje je prazna!");
    } else this.h1 = true;
  }
  ...
  showBooksToRead() {
    ...
    for (let i = 0; i < this.user.booksToRead.length; i++) {
      for (let j = 0; j < this.books.length; j++) {
        if (this.user.booksToRead[i] == this.books[j].id) {
          this.booksToRead.push(this.books[j]);
          break;
        }
      }
    }

    if (this.booksToRead.length == 0) {
      alert("Lista knjiga za citanje je prazna!");
    } else this.h2 = true;
  }
  ...
  showFinishedBooks() {
    ...
    for (let i = 0; i < this.user.booksFinished.length; i++) {
      for (let j = 0; j < this.books.length; j++) {
        if (this.user.booksFinished[i] == this.books[j].id) {
          this.booksFinished.push(this.books[j]);
          break;
        }
      }
    }

    if (this.booksFinished.length == 0) {
      alert("Lista knjiga za citanje je prazna!");
    } else this.h3 = true;
  }
  ...
  changeName() {
    ...
    this.userService
      .changeUserName(this.user, this.userName)
      .subscribe((data: boolean) => {
        if (data) {
  ...
  changeSurname() {
    ...
    this.userService
      .changeUserSurname(this.user, this.userSurname)
      .subscribe((data: boolean) => {
        if (data) {
  ...
  changeBirthday() {
    ...
    this.userService
      .changeUserBirthday(this.user, this.userBirthday)
      .subscribe((data: boolean) => {
        if (data) {
  ...
  changeCountry() {
    ...
    this.userService
      .changeUserCountry(this.user, this.userCountry)
      .subscribe((data: boolean) => {
        if (data) {
  ...
  changeCity() {
    ...
    this.userService
      .changeUserCity(this.user, this.userCity)
      .subscribe((data: boolean) => {
        if (data) {
  ...
  changeUsername() {
    ...
    this.userService
      .changeUserUsername(this.user, this.userUsername)
      .subscribe((data: boolean) => {
        if (data) {
  ...
  changeEmail() {
    ...
    this.userService
      .changeUserEmail(this.user, this.userEmail)
      .subscribe((data: boolean) => {
        if (data) {
  ...
  async makeHappening() {
    if (
      this.emptyField(this.happening.description) ||
      this.emptyField(this.happening.name)
    ) {
      this.u = false;
    ...
    if (this.happening.followers != null) {
      for (let i = 0; i < this.happening.followers.length; i++) {
        await new Promise((resolve, reject) => {
          this.userService
            .findUser(this.happening.followers[i])
            .subscribe((data: User) => {
              if (data) {
                data.notifications.push(
                  "Korisnik " +
                    this.user.username +
                    " vas je dodao u svoj dogadjaj " +
                    this.happening.name
                );

                this.userService
                  .updateNotifications(data)
                  .subscribe((data2: boolean) => {
                    if (data2) resolve();
                  });
              }
            });
    ...
    if (this.happening.dateEnd == null && this.happening.noEnd == false) {
      this.u = false;
      this.porukaHappening =
        "Ako ne upisete kraj onda morate reci da je dogadjaj nema kraja i obrnuto!";
      return;
    }
    ...
    if (this.user.userType == "moderator") {
      if (this.emptyField(this.happeningType)) {
        ...
        this.porukaHappening =
          "Moderator mora odabrati kakav tip desavanja pravi privatan ili javan";

        return;
      }
    } else {
      this.happeningType = "privatno";
    }
    ...
    if (this.happening.dateBegin != null && this.happening.dateEnd != null) {
      if (dateBegin.getTime() > dateEnd.getTime()) {
        this.porukaHappening =
          "Datum pocetka ne moze poceti kasnije od datuma kraja!";
        this.u = false;
        return;
      }
    }
